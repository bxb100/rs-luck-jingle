...
HTTP 服务会先预占监听地址；只有预占成功，才会初始化打印 session、应用所选浓度、将自动关机时间设为“手动关闭”，并打印一张紧凑的连接欢迎小票。欢迎小票在每次服务启动时只打印一次，后续 session 重连会重新应用配置但不会重复打印；若欢迎小票打印失败，服务不会开始接受 HTTP 请求。Android `2.7.19` 连接成功时只通过弹窗和设备名称提供界面反馈，并不会自动打印；启动欢迎小票是 Rust 服务新增的行为。该行为仅适用于 HTTP 服务主程序，下面的 `print_image` 直接图片测试不会额外打印欢迎小票。

服务运行期间由唯一的打印 worker 串行执行连接健康检查，不会让心跳与打印命令竞争 RFCOMM 回复。每隔 `30 s`，worker 使用 Status 请求 `10 FF 40` 作为 ping；`3 s` 内收到恰好一个合法状态字节即视为 pong，即使该状态表示缺纸、开盖或其他暂时不可打印条件。超时、I/O 错误或异常长度会关闭当前脏连接；空闲且断联时，每次探测周期都会尝试重连，并重新应用浓度和“手动关闭”设置。重连不会重复打印欢迎小票，也不会自动重放失败或结果未知的作业；新打印请求可以在下一个周期前触发一次更早的重连尝试。长时间渲染或打印期间不会插入探测，错过的周期在下一个安全边界只合并执行一次。
